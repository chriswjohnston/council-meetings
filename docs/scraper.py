...
def write_council_data_json(meetings_by_year, yt_videos={}):
    ...
    # ── Preserve existing meetings not found by scraper ──────────
    # Carry forward any meetings (e.g. 2022/2023) that the live site
    # no longer exposes as PDFs — keep their URLs, extra_docs, etc.
        ...
        try:
            ...
            for m in existing:
                if m["date"] not in scraped_dates:
                    ...
                    carried += 1
            if carried:
                print(f"  ✓ Carried forward {carried} existing meeting(s) not on live site")
        except Exception as e:
